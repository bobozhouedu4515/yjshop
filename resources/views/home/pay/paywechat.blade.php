        ...
        <div class="payOrder-warp" id="pay_main">
            <div class="payOrder-desc">
                    ...
                    <div class="pay-total">
                        ...
                        <img alt="模式二扫码支付" src="{{asset ('org/php_sdk_v3.0.9/example')}}/qrcode.php?data={{urlencode($url2)}}" style="width:150px;height:150px;"/>
                        <div style="color:#ff0000"></div>
                        <span>应付总额：</span>
                        <div class="pay-price">￥{{$order->total_price}}</div>
                    </div>

                <div class="pay-txt">
                    <p>订单号：<em id="nku">{{$order->number}}</em></p>
                    <p>您选定的支付方式为：<span id="paymode">微信支付</span></p>
                    <p>配送方式: <span id="express">顺丰速运</span></p>
                </div>
                <div class="o-list">
                    <div class="o-list-info">
                        <span id="shdz">寄送至:{{$address->province}}/{{$address->city}}{{$address->distinct}}/ {{$address->detail}}&nbsp;&nbsp;</span>
                        <span id="shr">收货人:{{$address->name}}</span>
                        <span id="mobile">联系电话:{{$address->tel_num}}</span>
                    </div>
                </div>
...
@push('js')
    ...
    $.post("{{route('pay.check_order_status')}}",{_token:'{{csrf_token ()}}',number:number},function (res) {
    if(res.code == 1){
    location.href = "{{route ('home.user.user_info',[auth ()->id ()])}}";
    }
    },'json')
    },1000)
    })
